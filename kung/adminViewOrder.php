...
if ($stmt = $mysqli->prepare("SELECT ordine.Id, ordine.Costo, ordine.BloccodiConsegna, ordine.Orario, ordine.Data, ordine.StatoOrdine, cliente.Nome, fornitore.nome FROM ordine, cliente, fornitore WHERE ordine.IdCliente = cliente.Username AND ordine.IdFornitore = fornitore.username AND ordine.StatoOrdine != '$state'")) {
...
if ($stmt->num_rows > 0) {
        while ($i < $stmt->num_rows) {
            $stmt->bind_result($id, $costo, $blocco, $orario, $data, $status, $nomeCliente, $nomeFornitore); // recupera le variabili dal risultato ottenuto.
            $stmt->fetch();
            ?>
    <tr>
    <td><?php echo $id; ?></td>
