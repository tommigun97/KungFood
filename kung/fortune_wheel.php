...
/*f ($stmtA = $mysqli->prepare("SELECT nome FROM cliente WHERE Username = ? LIMIT 1")) {
    $stmtA->bind_param('s', $userName); // esegue il bind del parametro '$email'.
    $stmtA->execute(); // esegue la query appena creata.
    $stmtA->store_result();
    $stmtA->bind_result($nome); // recupera il risultato della query e lo memorizza nelle relative variabili.
    $stmtA->fetch();
}*/
//if(login_check($mysqli) == true) {
  if($stmt = $mysqli->prepare("SELECT nome FROM fornitore WHERE banned ='0' ORDER BY RAND() LIMIT 6")){
...
<head>
  ...
  <link rel="stylesheet" href="../wheel-of-fortune/CSS/style.css" type="text/css">
  <link rel="stylesheet" href="../css/style.css" type="text/css">
  <script src='https://cdnjs.cloudflare.com/ajax/libs/jquery/3.1.1/jquery.min.js'></script>
  <script src='https://cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min.js'></script>
  <script src="../wheel-of-fortune/js/index.js" type="text/javascript"></script>
...
<body class="w3-display-container">
  ...
  <div class="w3-container w3-display-container w3-padding-16">
    ...
    <h3 class="w3-wide"><a href="./user/paginaTemplate.php">Kung Food</a></h3> <!--qui manca un regola css per togliere la sottolineatura-->
  </div>
  <?php
...
<a href="./user/allFornitori.php" class="w3-bar-item w3-button">Ristoranti</a>
...
<div class="w3-padding-64 w3-large " style="font-weight:bold">
<a href="./user/allFornitori.php" class="w3-bar-item w3-button">Ristoranti</a>
<a href="./user/getOrdini.php" class="w3-bar-item w3-button">Ordini</a>
<a href="./fortune_wheel.php" class="w3-bar-item w3-button">Ruota della Fortuna</a>
<a href="./user/Carrello.php" class="w3-bar-item w3-button">Carrello</a>
<a href="./user/getUser.php" class="w3-bar-item w3-button">Modifica account</a>
...
?>
  ...
  <div class="w3-main" style="margin-left:250px">
    ...
    <header class="w3-container w3-padding-large">
      <div id="text" class="w3-display-container w3-left">
         <p>Lascia scegliere, non al caso....<br /> che non esiste, ma alla ruota della fortuna,<br /> sarei lei a dirti
           dove
           dovresti mangiare oggi.</p>
       </div>
    ...
    <section class=" w3-container w3-padding-64 ">
      <div class="container">
        <button id="spin" class="w3-button w3-center w3-round-xxlarge w3-amber">Spin</button>
        <a href="./fortune_wheel.php">
        <button id="reset" class="w3-button w3-center w3-round-xxlarge w3-red">Reset</button></a>
        <div class="backdrop w3-right " id="backdrop" >
          <?php
          if($stmt->num_rows > 0) {
            while($i < $stmt->num_rows){
              $stmt->bind_result($nome); // recupera le variabili dal risultato ottenuto.
              $stmt->fetch();
              $i++;
          ?>
          <!-- If you add more segments here then you need to change the CSS angles -->
          <div class="segment segment-<?php echo$i ?>"></div>
          <div id="<?php echo $i?>"class="segment segment-<?php echo$i.$i ?> w3-left"><br/><?php echo substr($nome, 0, 20) ?></div>
          <?php
          }
        } else {
          echo 'error1';
      ...
      <div id="wrapper" class="result-wrapper">
        <p>La ruota ha deciso che mangerai da:<div id="result"></div></p>
      </div>
    </section>
  ...
  <!-- <div class="w3-col l3 s6">
   ...
   <div id="ViewLogin" class="w3-modal " style="display:none">
     <div class="w3-modal-content w3-card-4 w3-animate-zoom" style="max-width:400px">
       ...
       <div>
         <form id="logAffamato" class="w3-container"  method="post" action="./user/process_login.php">
         ...
         <form id="logFornitore" class="w3-container"  method="post" action="./user/process_login.php">
           <div class="w3-section">
         ...
         <form id="logAdmin" class="w3-container" method="post" action="./user/process_login.php">
           <div class="w3-section">
